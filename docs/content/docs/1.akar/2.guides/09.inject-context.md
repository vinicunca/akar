...
description: Utilize `injectContext` to enhance component composition in Akar, enabling powerful and flexible UI development.
---

::warning

Use with caution! API might change

Akar exposes internal `injectContext` to further extend the ability to compose and construct complex components. However, this API was primarily designed to be internal use. Thus the API might change without notice.

::

## Introduction to `injectContext`

In Akar, all root component, and some other component exports an `injectContext` function, which is a key feature for managing component state and enabling seamless composition. This guide will show you how to craft your own child component based on the provided context.

## What is `injectContext`?

`injectContext` is a function provided by each Akar component that allows you to access the internal state and methods of that component.
