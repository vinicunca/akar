...
links:
  ...
  - label: GitHub
    ...
    to: https://github.com/vinicunca/akar/blob/main/packages/pohon/src/runtime/components/editor.vue
---
...
::callout{icon="i-simple-icons-github" to="https://github.com/akar/blob/main/docs/app/components/content/examples/editor/editor-example.vue" aria-label="View source code"}
...
export default defineNuxtConfig({
  vite: {
    optimizeDeps: {
      include: [
        'pohon-ui > prosemirror-state',
        'pohon-ui > prosemirror-transform',
        'pohon-ui > prosemirror-model',
        'pohon-ui > prosemirror-view',
        'pohon-ui > prosemirror-gapcursor'
      ]
    }
  }
});
...
```vue
<template>
  <PEditor :placeholder="{ placeholder: 'Start writing...', mode: 'firstLine' }" />
</template>
```
::
...
```vue
<template>
  <PEditor :placeholder="{ placeholder: 'Start writing...', includeChildren: true }" />
</template>
```
::
...
Handlers wrap TipTap's built-in commands to provide a unified interface for editor actions. When you add a `kind` property to a [EditorToolbar](/docs/pohon/components/editor-toolbar) or [EditorSuggestionMenu](/docs/pohon/components/editor-suggestion-menu) item, the corresponding handler executes the TipTap command and manages its state (active, disabled, etc.).
...
const items: Array<EditorToolbarItem> = [
...
] satisfies Array<EditorToolbarItem<typeof customHandlers>>;
</script>
...
You can use the [EditorToolbar](/docs/pohon/components/editor-toolbar) component to add a `fixed`, `bubble`, or `floating` toolbar to the Editor with common formatting actions.
...
You can use the [EditorDragHandle](/docs/pohon/components/editor-drag-handle) component to add a draggable handle for reordering blocks.
...
You can use the [EditorSuggestionMenu](/docs/pohon/components/editor-suggestion-menu) component to add slash commands for quick formatting and insertions.
...
You can use the [EditorMentionMenu](/docs/pohon/components/editor-mention-menu) component to add @ mentions for tagging users or entities.
...
You can use the [EditorEmojiMenu](/docs/pohon/components/editor-emoji-menu) component to add emoji picker support.
...
1. Create a Vue component that uses the [FileUpload](/docs/pohon/components/file-upload) component:
