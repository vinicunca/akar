...
description: Pohon UI's design system uses Tailwind CSS for simple theming and easy customization.
...
Pohon UI's color system is designed around **semantic naming** rather than specific color values. This approach makes your UI more maintainable and allows for easy theme switching.

### Semantic colors

Pohon UI provides semantic color aliases that describe the **purpose** of the color. Each alias is defined based on a color from your `@theme` configuration, which can be any color you define in addition to the [default Tailwind CSS palette](https://tailwindcss.com/docs/colors).
...
These semantic colors are available in the `color` prop of Pohon UI components:
...
#nuxt
:::div
You can configure these colors at runtime in your [`app.config.ts`](https://nuxt.com/docs/guide/directory-structure/app-config#app-config-file) file under the `pohon.colors` key, allowing for dynamic theme customization without restarting the server:
...
#vue
:::div
You can configure these colors at runtime in your `vite.config.ts` file under the `pohon.colors` key, allowing for dynamic theme customization:
...
#nuxt
:::div
First, register the new color in your `nuxt.config.ts` under the `pohon.theme.colors` key:
...
Then, assign it in your `app.config.ts` under the `pohon.colors` key:
