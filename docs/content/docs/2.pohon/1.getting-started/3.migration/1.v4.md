...
The `ButtonGroup` component has been renamed to [`FieldGroup`](/docs/pohon/components/field-group):
...
+ <PFieldGroup>
    ...
    <PButton icon="i-lucide:chevron-down" />
+ </PFieldGroup>
- </PButtonGroup>
</template>
```
...
The `PageMarquee` component has been renamed to [`Marquee`](/docs/pohon/components/marquee):

```diff
<template>
- <UPageMarquee :items="items" />
+ <PMarquee :items="items" />
</template>
```

### Removed PageAccordion

The `PageAccordion` component has been removed in favor of [`Accordion`](/docs/pohon/components/accordion):
...
+   :pohon="{ trigger: 'text-base', body: 'text-base text-muted' }"
  />
</template>
```
...
The `modelModifiers` shape used by [`Input`](/docs/pohon/components/input), [`InputNumber`](/docs/pohon/components/input-number) and [`Textarea`](/docs/pohon/components/textarea) has changed in v4:
...
```diff
- <PInput v-model.nullify="value" />
+ <PInput v-model.nullable="value" />
```

```diff
- <PTextarea v-model="value" :model-modifiers="{ nullify: true }" />
+ <PTextarea v-model="value" :model-modifiers="{ nullable: true }" />
```
...
3. **Nested forms should now provide a `name`** prop (similar to `PFormField`) and will automatically inherit their state from their parent form.

```diff
<template>
  <PForm :state="state" :schema="schema" @submit="onSubmit">
...
+       nested
      >
        <PFormField :label="!index ? 'Description' : undefined" name="description">
          <PInput v-model="item.description" />
        </PFormField>
        <PFormField :label="!index ? 'Price' : undefined" name="price">
