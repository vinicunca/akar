...
:docs-pohon-props{slug="pre" prose label="Props"}

::docs-pohon-theme{slug="pre" prose label="Theme"}
---
extra:
  - codeIcon
---
...
::tip
By default for syntax highlighting, `one-dark-pro` themes are used for both light & dark mode. You can change this in your `nuxt.config.ts` through the [`content.build.markdown.highlight`](https://content.nuxt.com/docs/getting-started/configuration#highlight) key.
::
...
::tip
You can change the icon in your `app.config.ts` through the `pohon.icons.copy` and `pohon.icons.copyCheck` keys:
...
:docs-pohon-props{slug="code" prose label="Props"}

:docs-pohon-theme{slug="code" prose label="Theme"}
