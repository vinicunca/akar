...
export const contextMenuTheme = {
  slots: {
    content: 'min-w-32 bg-background shadow-lg rounded-md ring ring-ring overflow-hidden animate-in fade-in-80 data-[state=closed]:(animate-out fade-out-0 zoom-out-95) data-[state=open]:(animate-in fade-in-0 zoom-in-95) data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 origin-$akar-context-menu-content-transform-origin flex flex-col',
    ...
    item: 'group relative w-full flex items-start select-none outline-none before:absolute before:-z-1 before:inset-px before:rounded-md data-[disabled]:(cursor-not-allowed opacity-75)',
...
} satisfies UvConfig<typeof pohon>['contextMenu'];
