...
export interface ATooltipRootProps {
  ...
  disabled?: boolean;
  /**
   * Prevents ATooltip.Content from remaining open when hovering.
   * Disabling this has accessibility consequences. Inherits
   * from ATooltip.Provider.
   */
...
export type ATooltipRootEmits = {
  /** Event handler called when the open state of the tooltip changes. */
  'update:open': [value: boolean];
};

export interface ATooltipContext {
  contentId: string;
...
export const [
  injectATooltipRootContext,
  provideATooltipRootContext,
] = createContext<ATooltipContext>('ATooltipRoot');
</script>
...
const disableATooltip = computed(() =>
  props.disabled ?? providerContext.disabled.value,
);
