...
export interface AListboxVirtualizerProps<T extends AcceptableValue = AcceptableValue> {
...
rootContext.virtualFocusHook.on((event) => {
  const index = props.options.findIndex((option) => {
    if (Array.isArray(rootContext.modelValue.value)) {
      return compare({
        value: option,
        currentValue: rootContext.modelValue.value[0],
        comparator: rootContext.by,
      });
    } else {
      return compare({
        value: option,
        currentValue: rootContext.modelValue.value!,
        comparator: rootContext.by,
      });
    }
  });
  if (index !== -1) {
    ...
    requestAnimationFrame(() => {
      ...
      if (item) {
        rootContext.changeHighlight({ el: item });
        if (event) {
          item?.focus();
        }
      }
...
function handleMultipleReplace(event: Event, intent: 'first' | 'last' | 'next' | 'prev') {
  if (
    !rootContext.firstValue?.value
  ...
  const collection = getItems().filter((item) => item.ref.dataset.disabled !== '');
  const lastValue = collection.find(
    (item) => item.ref === rootContext.highlightedElement.value,
  )?.value;
...
rootContext.virtualKeydownHook.on((event) => {
  ...
  if (['first', 'last'].includes(intent)) {
    ...
    requestAnimationFrame(() => {
      ...
      if (item) {
        rootContext.changeHighlight({ el: item.ref });
      }
    });
