...
import ACalendar from './story/_a-calendar.vue';
import ACalendarMultiple from './story/_a-calendar-multiple.vue';
...
describe('calendar - `multiple`', () => {
  it('handles default value when `value` prop is provided - `CalendarDate[]`', async () => {
    ...
    } as { calendarProps: { multiple: true } & ACalendarRootProps });
  ...
  it('sets placeholder to last value in `value` prop', async () => {
    ...
    const { calendar, rerender, getByTestId } = setupMulti({
      calendarProps: { modelValue: [d1, d2] } as { multiple: true } & ACalendarRootProps,
