...
import { addUnlinkedScrollListener } from './a-scroll.utils';
...
const scrollbarContextVisible = injectAScrollAreaScrollbarVisibleContext();
...
function handleScroll() {
  if (!removeUnlinkedScrollListenerRef.value) {
    const listener = addUnlinkedScrollListener({
      node: viewport.value!,
      handler: scrollbarContextVisible.onThumbPositionChange,
    });
