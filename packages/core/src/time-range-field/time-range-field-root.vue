...
type ATimeRangeFieldRootContext = {
  locale: Ref<string>;
...
export interface ATimeRangeFieldRootProps extends APrimitiveProps, FormFieldProps {
  ...
  isTimeUnavailable?: DateMatcher;
}

export type ATimeRangeFieldRootEmits = {
...
export const [
  injectATimeRangeFieldRootContext,
  provideATimeRangeFieldRootContext,
] = createContext<ATimeRangeFieldRootContext>('ATimeRangeFieldRoot');
</script>
...
import { areAllDaysBetweenValid, isDateBefore, isDateBeforeOrSame } from '../date';
...
defineOptions({
  name: 'ATimeRangeFieldRoot',
  inheritAttrs: false,
});

const props = withDefaults(
  defineProps<ATimeRangeFieldRootProps>(),
  {
...
defineSlots<{
  default?: (props: {
    ...
    segments: { start: Array<{ part: SegmentPart; value: string }>; end: Array<{ part: SegmentPart; value: string }> };
...
const isStartInvalid = computed(() => {
  ...
  if (convertedMinValue.value && isDateBefore(convertedStartValue, convertedMinValue.value)) {
    return true;
  }

  if (convertedMaxValue.value && isDateBefore(convertedMaxValue.value, convertedStartValue)) {
    return true;
  }
...
const isEndInvalid = computed(() => {
  ...
  if (convertedMinValue.value && isDateBefore(convertedEndValue, convertedMinValue.value)) {
    return true;
  }

  if (convertedMaxValue.value && isDateBefore(convertedMaxValue.value, convertedEndValue)) {
    return true;
  }
...
const isInvalid = computed(() => {
  ...
  if (!isDateBeforeOrSame(convertedModelValue.start, convertedModelValue.end)) {
    return true;
  }

  if (propsIsTimeUnavailable.value !== undefined) {
    const allValid = areAllDaysBetweenValid({
      start: convertedModelValue.start,
      end: convertedModelValue.end,
