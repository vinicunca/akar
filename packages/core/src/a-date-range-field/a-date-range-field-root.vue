...
export type ADateRangeType = 'end' | 'start';

type ADateRangeFieldRootContext = {
  ...
  segmentContents: Ref<{
    end: Array<{ part: DateTimeSegmentPart; value: string }>;
    start: Array<{ part: DateTimeSegmentPart; value: string }>;
  }>;
  segmentValues: Record<ADateRangeType, Ref<DateTimeSegmentValueObj>>;
...
export const [
  ...
  provideADateRangeFieldRootContext,
] = createContext<ADateRangeFieldRootContext>('ADateRangeFieldRoot');
</script>
...
const isStartInvalid = computed(() => {
  ...
  if (props.minValue
    && isDateBefore(modelValue.value.start, props.minValue)) {
    return true;
  }
...
const isEndInvalid = computed(() => {
  ...
  if (props.minValue
    && isDateBefore(modelValue.value.end, props.minValue)
  ) {
    return true;
  }
...
const startSegmentValues = ref<DateTimeSegmentValueObj>(modelValue.value.start
  ? { ...syncSegmentValues({ value: modelValue.value.start, formatter }) }
  : { ...initialSegments });
const endSegmentValues = ref<DateTimeSegmentValueObj>(modelValue.value.end
  ? { ...syncSegmentValues({ value: modelValue.value.end, formatter }) }
  : { ...initialSegments });

const startSegmentContent = computed(() => createDateContent({
...
const endSegmentContent = computed(() => createDateContent({
  granularity: inferredGranularity.value,
  dateRef: placeholder.value,
  formatter,
  hideTimeZone: props.hideTimeZone,
...
watch(modelValue, (_modelValue) => {
  if (_modelValue.start !== undefined
    && (
...
const currentSegmentIndex = computed(() =>
  Array.from(segmentElements.value)
    .findIndex((el) =>
