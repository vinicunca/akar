...
const props = withDefaults(
  defineProps<ANavigationMenuViewportProps>(),
  {
    align: 'center',
  },
);
  ...
  >
    <APrimitive
      ...
      :style="{
        ...
        ['--akar-navigation-menu-viewport-width']: size ? `${size?.width}px` : undefined,
        ['--akar-navigation-menu-viewport-height']: size ? `${size?.height}px` : undefined,
        ['--akar-navigation-menu-viewport-left']: position ? `${position?.left}px` : undefined,
        ['--akar-navigation-menu-viewport-top']: position ? `${position?.top}px` : undefined,
      }"
      :hidden="!present"
