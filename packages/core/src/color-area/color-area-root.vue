...
export interface AColorAreaRootProps extends APrimitiveProps, FormFieldProps {
...
export type AColorAreaRootEmits = {
...
export interface AColorAreaRootContext {
  color: Ref<Color>;
  xValue: Ref<number>;
  yValue: Ref<number>;
...
export const [
  injectAColorAreaRootContext,
  provideAColorAreaRootContext,
] = createContext<AColorAreaRootContext>('AColorAreaRoot');
</script>
...
const emits = defineEmits<AColorAreaRootEmits>();
...
if (colorSpace.value === 'hsl') {
  initialHueValue = convertToHsl(color.value).h;
} else if (colorSpace.value === 'hsb') {
  initialHueValue = convertToHsb(color.value).h;
}
const hueValue = ref(initialHueValue);
