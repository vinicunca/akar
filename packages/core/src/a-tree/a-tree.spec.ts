...
import AATree from './story/_ATree.vue'
...
describe('given a AATree with a custom children structure', () => {
  ...
  beforeEach(async () => {
    document.body.innerHTML = ''
    wrapper = mount(AATree, { props: { items: customItems, multiple: true, selectionBehavior: 'toggle', getChildren: val => (!val.files) ? val.directories : (!val.directories) ? val.files : [...val.directories, ...val.files] }, attachTo: document.body })
    await nextTick()
    // content = wrapper.find('[role=tree]')
    items = wrapper.findAll('[role=treeitem]')
  })
