...
export type SelectEvent<T> = CustomEvent<{ isExpanded: boolean; isSelected: boolean; originalEvent: KeyboardEvent | PointerEvent; value?: T }>;
export type ToggleEvent<T> = CustomEvent<{ isExpanded: boolean; isSelected: boolean; originalEvent: KeyboardEvent | PointerEvent; value?: T }>;
...
import { injectATreeRootContext } from './a-tree-root.vue';
...
const isIndeterminate = computed(() => {
  if (
    ...
    return !children.every((child) =>
      rootContext.modelValue.value.find((v: any) =>
        rootContext.getKey(v) === rootContext.getKey(child),
      ),
    );
  } else {
    return undefined;
...
function handleKeydownRight(ev: KeyboardEvent) {
  ...
  if (isExpanded.value) {
    ...
    const nextElement = list.find((el) =>
      Number(el.getAttribute('data-indent')) === (props.level + 1),
    );
...
function handleKeydownLeft(ev: KeyboardEvent) {
  if (isExpanded.value) {
    ...
    const parentElement = list.find((el) =>
      Number(el.getAttribute('data-indent')) === (props.level - 1),
    );
