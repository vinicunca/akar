...
describe('given a ATagsInput with objects', async () => {
  let wrapper: VueWrapper<InstanceType<typeof ATagsInputObject>>;
  let input: DOMWrapper<HTMLInputElement>;
  let tags: Array<DOMWrapper<HTMLElement>>;
  ...
  describe('should be able to convert the value', () => {
    beforeEach(() => {
      ...
      wrapper = mount(ATagsInputObject, {
        props: {
  ...
  it('should throw an error if props are not ok', async () => {
    ...
    const wrapper = mount(ATagsInputObject, {
      global: {
        config: {
