...
function handlePointerDown(event: ResizeEvent) {
  ...
  if (intersectingHandles.length > 0) {
    updateResizeHandlerStates({ action: 'down', event });
...
function handlePointerMove(event: ResizeEvent) {
  ...
  updateResizeHandlerStates({ action: 'move', event });
...
function handlePointerUp(event: ResizeEvent) {
  ...
  updateResizeHandlerStates({ action: 'up', event });
  recalculateIntersectingHandles({ target, x, y });
  updateCursor();
      ...
      if (
        ...
        while (currentElement) {
          if (currentElement.contains(dragHandleElement)) {
            ...
            intersects({
              rectOne: currentElement.getBoundingClientRect(),
              rectTwo: dragHandleRect,
              strict: true,
            })
          ) {
            didIntersect = true;
...
function updateResizeHandlerStates(
  { action, event }:
  { action: ResizeHandlerAction; event: ResizeEvent },
) {
  registeredResizeHandlers.forEach((data) => {
