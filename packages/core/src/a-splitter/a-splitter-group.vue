...
export type PanelGroupContext = {
  ...
  getPanelStyle: (params: { defaultSize?: number; panelData: PanelData }) => CSSProperties;
  groupId: string;
  isPanelCollapsed: (panelData: PanelData) => boolean;
  isPanelExpanded: (panelData: PanelData) => boolean;
  panelGroupElement: Ref<null | ParentNode>;
  reevaluatePanelConstraints: (params: { panelData: PanelData; prevConstraints: PanelConstraints }) => void;
  registerPanel: (panelData: PanelData) => void;
  registerResizeHandle: (dragHandleId: string) => ResizeHandler;
  resizePanel: (params: { panelData: PanelData; unsafePanelSize: number }) => void;
...
const props = withDefaults(
  defineProps<ASplitterGroupProps>(),
  {
    autoSaveId: null,
    keyboardResizeBy: 10,
...
const TRIGGER_IMPERATIVE_API = 'imperative-api';
...
watchEffect(() => {
  ...
  if (autoSaveId) {
    ...
    const debouncer = debounce(
      savePanelGroupState,
      { waitMs: LOCAL_STORAGE_DEBOUNCE_INTERVAL },
    );

    // Limit the frequency of localStorage updates.
    if (!debouncedSave) {
      debouncedSave = debouncer.call;
    ...
    debouncedSave({
      autoSaveId,
      panels: clonedPanelDataArray,
      panelSizesBeforeCollapse: clonedPanelSizesBeforeCollapse,
      sizes: layout.value,
      storage: props.storage,
    });
  }
...
watch(
  ...
  () => {
    if (eagerValuesRef.value.panelDataArrayChanged) {
      ...
      let unsafeLayout: Array<number> | null = null;

      if (autoSaveId) {
        const state = loadPanelGroupState({ autoSaveId, panels: panelDataArray, storage });
        if (state) {
      ...
      if (unsafeLayout === null) {
        unsafeLayout = calculateUnsafeDefaultLayout(panelDataArray);
      }
  ...
  return function resizeHandler(event: ResizeEvent) {
    ...
    if (
      ...
      if (!layoutChanged) {
        ...
        reportConstraintsViolation({ resizeHandleId: dragHandleId, flag: 0 });
      }
    }
...
function startDragging(dragHandleId: string, event: ResizeEvent) {
  ...
  const handleElement = getResizeHandleElement({
    id: dragHandleId,
    scope: panelGroupElementRef.value,
  });
...
function panelDataHelper(
  { panelDataArray, panelData, layout }:
  {
    layout: Array<number>;
    panelData: PanelData;
    panelDataArray: Array<PanelData>;
