...
export function useDrawerSnapPoints(options: {
  snapPoints: Ref<Array<DrawerSnapPoint> | undefined>;
  ...
  const { snapPoints, activeSnapPoint, popupHeight, onSnapPointChange } = options;
  ...
  const resolvedSnapPoints = computed<Array<ResolvedSnapPoint>>(() => {
    ...
    const resolved: Array<ResolvedSnapPoint> = [];
