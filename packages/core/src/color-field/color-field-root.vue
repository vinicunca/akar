...
export interface AColorFieldRootProps extends APrimitiveProps, FormFieldProps {
...
export type AColorFieldRootEmits = {
  'update:modelValue': [value: string];
  'update:color': [value: Color];
};

export interface AColorFieldRootContext {
  color: Ref<Color>;
...
export const [
  injectAColorFieldRootContext,
  provideAColorFieldRootContext,
] = createContext<AColorFieldRootContext>('AColorFieldRoot');
</script>
...
const emits = defineEmits<AColorFieldRootEmits>();

const { colorSpace, channel, disabled, readonly, disableWheelChange, placeholder, step: stepProp } = toRefs(props);
...
function commit() {
  ...
  if (channel.value) {
    ...
    const numValue = Number.parseFloat(inputValue.value);
