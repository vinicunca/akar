...
import type { AYearRangePickerRootProps } from './year-range-picker-root.vue';
...
import AYearRangePicker from './story/_year-range-picker.vue';
...
function setup(props: { pickerProps?: AYearRangePickerRootProps; emits?: { 'onUpdate:modelValue'?: (data: DateRange) => void } } = {}) {
  const user = userEvent.setup();
  const returned = render(AYearRangePicker, { props });
...
it('should pass axe accessibility tests', async () => {
  const wrapper = mount(AYearRangePicker);
