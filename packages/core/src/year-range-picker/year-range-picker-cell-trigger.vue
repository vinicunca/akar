...
export interface AYearRangePickerCellTriggerProps extends APrimitiveProps {
  /** The date value provided to the cell trigger */
  year: DateValue;
}

export interface AYearRangePickerCellTriggerSlot {
  default?: (props: {
    /** Current year value */
    yearValue: string;
...
defineSlots<AYearRangePickerCellTriggerSlot>();
...
const yearValue = computed(() => {
  ...
  return rootContext.formatter.fullYear({ date: toDate(props.year) });
});

const labelText = computed(() => {
  // eslint-disable-next-line ts/no-unused-expressions
  rootContext.locale.value;
  return rootContext.formatter.custom({ date: toDate(props.year), options: { year: 'numeric' } });
});
...
function changeYear(event: MouseEvent | KeyboardEvent, date: DateValue) {
...
<template>
  <APrimitive
    ...
    data-akar-year-range-picker-cell-trigger
