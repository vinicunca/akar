...
function handleInput(event: InputEvent) {
  ...
  updateModelValueAt({ index: props.index, value: target.value });
...
function handleBackspace(event: KeyboardEvent) {
  ...
  if (value) {
    updateModelValueAt({ index: props.index, value: '' });
  } else {
    const prevEl = inputElements.value[props.index - 1];

    if (prevEl) {
      prevEl.focus();
      updateModelValueAt({ index: props.index - 1, value: '' });
    }
  }
}
...
function handleMultipleCharacter(values: string) {
  ...
  const initialIndex = values.length >= inputElements.value.length
    ? 0
    : props.index;
