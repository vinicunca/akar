...
import type { ATimeFieldRootProps } from './a-time-field-root.vue';

import TimeField from './story/_a-time-field.vue';
...
function isDaylightSavingsTime(): boolean {
  ...
  return timezoneOffset < Math.max(january.getTimezoneOffset(), july.getTimezoneOffset());
}

function thisTimeZone(date: string): string {
  return Intl.DateTimeFormat('en-US', { timeZoneName: 'short' }).formatToParts(new Date(date)).find((p) => p.type === 'timeZoneName')?.value ?? '';
}

function setup(props: { emits?: { 'onUpdate:modelValue'?: (data: TimeValue) => void }; timeFieldProps?: ATimeFieldRootProps } = {}) {
