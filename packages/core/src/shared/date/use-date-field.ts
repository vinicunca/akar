...
function monthSegmentAttrs(props: SegmentAttrProps) {
  ...
  const valueMin = 1;
  const valueMax = 12;
  const valueText = isEmpty ? 'Empty' : `${valueNow} - ${formatter.fullMonth({ date: toDate(date) })}`;
...
export function useDateField(props: UseDateFieldProps) {
  ...
  function updateDayOrMonth(max: number, num: number, prev: null | number) {
  ...
  function updateMinuteOrSecond(num: number, prev: null | number) {
    const max = 59;
    let moveToNext = false;
    const maxStart = Math.floor(max / 10);
  ...
  function updateYear(num: number, prev: null | number) {
    let moveToNext = false;
  ...
  function handleDaySegmentKeydown(event: KeyboardEvent) {
    ...
    if (event.key === KEY_CODES.ARROW_DOWN || event.key === KEY_CODES.ARROW_UP) {
      props.segmentValues.value.day = dateTimeValueIncrementation({ event, part: 'day', dateRef: props.placeholder.value, prevValue });
      return;
    }
  ...
  function handleMonthSegmentKeydown(event: KeyboardEvent) {
    ...
    if (event.key === KEY_CODES.ARROW_DOWN || event.key === KEY_CODES.ARROW_UP) {
      props.segmentValues.value.month = dateTimeValueIncrementation({ event, part: 'month', dateRef: props.placeholder.value, prevValue });
      return;
    }
  ...
  function handleYearSegmentKeydown(event: KeyboardEvent) {
    ...
    if (event.key === KEY_CODES.ARROW_DOWN || event.key === KEY_CODES.ARROW_UP) {
      props.segmentValues.value.year = dateTimeValueIncrementation({ event, part: 'year', dateRef: props.placeholder.value, prevValue });
      return;
    }
  ...
  function handleMinuteSegmentKeydown(event: KeyboardEvent) {
    ...
    if (!isAcceptableSegmentKey(event.key) || isSegmentNavigationKey(event.key) || !('minute' in dateRef) || !('minute' in props.segmentValues.value)) {
      return;
    }
    ...
    if (event.key === KEY_CODES.ARROW_UP || event.key === KEY_CODES.ARROW_DOWN) {
      props.segmentValues.value.minute = minuteSecondIncrementation({ event, part: 'minute', dateRef: props.placeholder.value, prevValue });
    }
  ...
  function handleSecondSegmentKeydown(event: KeyboardEvent) {
    ...
    if (!isAcceptableSegmentKey(event.key) || isSegmentNavigationKey(event.key) || !('second' in dateRef) || !('second' in props.segmentValues.value)) {
      return;
    }
    ...
    if (event.key === KEY_CODES.ARROW_UP || event.key === KEY_CODES.ARROW_DOWN) {
      props.segmentValues.value.second = minuteSecondIncrementation({ event, part: 'second', dateRef: props.placeholder.value, prevValue });
    }
