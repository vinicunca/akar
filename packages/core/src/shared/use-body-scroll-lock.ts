  ...
  watch(
    ...
    (val, oldVal) => {
      ...
      if (context.scrollBody?.value) {
        if (isPlainObject(context.scrollBody.value)) {
          config = defu(
            {
              padding: context.scrollBody.value.padding === true
                ? verticalScrollbarWidth
                : context.scrollBody.value.padding,
              margin: context.scrollBody.value.margin === true
                ? verticalScrollbarWidth
                : context.scrollBody.value.margin,
            },
            defaultConfig,
          );
        } else {
      ...
      if (isIOS) {
        stopTouchMoveListener = useEventListener(
          document,
          'touchmove',
          (event: TouchEvent) => preventDefault(event),
          { passive: false },
        );
      }
