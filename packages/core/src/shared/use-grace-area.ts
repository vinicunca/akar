...
import type { Side } from '~~/a-popper/utils';
...
// Reset the inTransit state if idle/scrolled.
  ...
  watchEffect((cleanupFn) => {
    if (triggerElement.value && containerElement.value) {
      const handleTriggerLeave = (event: PointerEvent) => handleCreateGraceArea({ event, hoverTarget: containerElement.value! });
      const handleContentLeave = (event: PointerEvent) => handleCreateGraceArea({ event, hoverTarget: triggerElement.value! });
    ...
    if (pointerGraceArea.value) {
      const handleTrackPointerGrace = (event: PointerEvent) => {
        ...
        const isPointerOutsideGraceArea = !isPointInPolygon({
          ...
          polygon: pointerGraceArea.value,
        });
...
function getPaddedExitPoints(
  { exitPoint, exitSide, padding = 5 }:
  { exitPoint: Point; exitSide: Side; padding?: number },
) {
  const paddedExitPoints: Array<Point> = [];
  switch (exitSide) {
    case 'bottom':
