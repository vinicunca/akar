 ...
 * array.
 * @param options
 * @param options.base - It represents the base value that you want to compare with the `current` value.
 * @param options.current - The `current` parameter represents the current value that you want to compare with the `base` value or values.
 ...
 */
export function isValueEqualOrExist<T>(
  { base, current }: { base: Params<T>; current: Params<T> },
) {
  if (isNullish(base)) {
    return false;
  }
