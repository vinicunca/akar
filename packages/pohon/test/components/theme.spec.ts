...
describe('Theme', () => {
  ...
  it('nested theme overrides outer theme', async () => {
    const wrapper = await mountSuspended({
      components: { Theme, Button },
      template: `
        <Theme :pohon="{ button: { base: 'outer-theme-class' } }">
          <Theme :pohon="{ button: { base: 'inner-theme-class' } }">
            <Button label="Themed" />
          </Theme>
        </Theme>
      `,
  ...
  it('deeply nested themes (3 levels)', async () => {
    const wrapper = await mountSuspended({
      components: { Theme, Button },
      template: `
        <Theme :pohon="{ button: { base: 'level-1-class' } }">
          <Theme :pohon="{ button: { base: 'level-2-class' } }">
            <Theme :pohon="{ button: { base: 'level-3-class' } }">
              <Button label="Themed" />
            </Theme>
          </Theme>
        </Theme>
  ...
  it('applies theme to multiple children of same type', async () => {
    const wrapper = await mountSuspended({
      components: { Theme, Button },
      template: `
        <Theme :pohon="{ button: { base: 'shared-theme-class' } }">
          <Button label="First" />
          <Button label="Second" />
        </Theme>
      `,
  ...
  it('applies theme to different component types simultaneously', async () => {
    const wrapper = await mountSuspended({
      ...
      template: `
        <Theme :pohon="{ button: { base: 'button-theme-class' }, badge: { base: 'badge-theme-class' } }">
  ...
  it('theme does not leak outside its scope', async () => {
    const wrapper = await mountSuspended({
      ...
      template: `
        <div>
          <Theme :pohon="{ button: { base: 'inside-theme-class' } }">
  ...
  it('applies theme to input component', async () => {
    const wrapper = await mountSuspended({
      ...
      template: `
        <Theme :pohon="{ input: { root: 'input-theme-class' } }">
