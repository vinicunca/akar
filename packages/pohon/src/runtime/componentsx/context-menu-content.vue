...
interface ContextMenuContentProps<T extends ArrayOrNested<PContextMenuItem>> extends Omit<AContextMenuContentProps, 'as' | 'asChild' | 'forceMount'> {
  items?: T;
...
interface ContextMenuContentEmits extends AContextMenuContentEmits {}
</script>
...
const contentProps = useForwardPropsEmits(reactiveOmit(props, 'sub', 'items', 'portal', 'labelKey', 'descriptionKey', 'checkedIcon', 'loadingIcon', 'externalIcon', 'class', 'pohon', 'pohonOverride'), emits);
const getProxySlots = () => omit(slots, ['default']);

const [DefineItemTemplate, ReuseItemTemplate] = createReusableTemplate<{ item: PContextMenuItem; active?: boolean; index: number }>();
...
<template>
  <DefineItemTemplate v-slot="{ item, active, index }">
    ...
    >
      ...
      <span
        v-if="(getProp(item, props.labelKey as string) || !!slots[(item.slot ? `${item.slot}-label` : 'item-label') as keyof PContextMenuSlots<T>]) || (getProp(item, props.descriptionKey as string) || !!slots[(item.slot ? `${item.slot}-description` : 'item-description') as keyof PContextMenuSlots<T>])"
      ...
      >
        ...
        <span
          v-if="getProp(item, props.descriptionKey as string) || !!slots[(item.slot ? `${item.slot}-description` : 'item-description') as keyof PContextMenuSlots<T>]"
