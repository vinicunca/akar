...
import PDropdownMenu from './DropdownMenu.vue';
...
const rootProps = useForwardProps(reactiveOmit(props, 'as', 'color', 'variant', 'activeColor', 'activeVariant', 'size', 'items', 'layout', 'editor', 'class', 'pohon'));
...
const pohon = computed(() => uv({ extend: uv(theme), ...(appConfig.pohon?.editorToolbar || {}) })({
  layout: props.layout,
}));
...
const groups = computed(() =>
  ...
  props.items?.length
    ? isArrayOfArray(props.items)
      ? props.items
      : [props.items as Array<NestedItem<T>>]
    : [],
);
...
function getDropdownProps(item: EditorToolbarDropdownItem) {
  const baseProps = pick(item as any, ['size', 'checkedIcon', 'loadingIcon', 'externalIcon', 'content', 'arrow', 'portal', 'modal', 'pohon']);
...
function mapDropdownItem(item: EditorToolbarDropdownChildItem): PDropdownMenuItem {
