...
const editorProps = computed(() => defu(props.editorProps, {
  attributes: {
    ...
    autocapitalize: 'off',
    ...attrs,
    class: pohon.value.base({ class: pohonProp.value?.base }),
  },
} as EditorOptions['editorProps']));
const contentType = computed(() => props.contentType || (isString(props.modelValue) ? 'html' : 'json'));
...
const mention = computed(() => defu(
  isBoolean(props.mention) ? {} : props.mention,
  {
    HTMLAttributes: {
      class: 'mention',
    },
