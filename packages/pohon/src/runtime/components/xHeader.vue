...
<template>
  ...
  <DefineLeftTemplate>
    <div :class="pohon.left({ class: props.pohon?.left })">
      ...
      <slot name="left">
        <PLink :to="to" :aria-label="ariaLabel" :class="pohon.title({ class: props.pohon?.title })">
          <slot name="title">
            {{ title }}
          </slot>
        </PLink>
  ...
  <APrimitive :as="as" v-bind="$attrs" :class="pohon.root({ class: [props.pohon?.root, props.class] })">
    <slot name="top" />
  ...
  >
    <template #content>
      <slot name="content">
        <div v-if="mode !== 'drawer'" :class="pohon.header({ class: props.pohon?.header })">
          <ReuseLeftTemplate />
