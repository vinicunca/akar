...
<script lang="ts">
import type { TabsRootProps, TabsRootEmits } from 'akar'
...
import { TabsRoot, TabsList, TabsIndicator, TabsTrigger, TabsContent, useForwardPropsEmits } from 'akar'
...
<template>
  ...
  >
    <TabsList :class="pohon.list({ class: props.pohon?.list })">
      ...
      >
        <slot name="leading" :item="item" :index="index">
          <PIcon v-if="item.icon" :name="item.icon" :class="pohon.leadingIcon({ class: [props.pohon?.leadingIcon, item.ui?.leadingIcon] })" />
          <PAvatar v-else-if="item.avatar" :size="((item.ui?.leadingAvatarSize || props.pohon?.leadingAvatarSize || pohon.leadingAvatarSize()) as AvatarProps['size'])" v-bind="item.avatar" :class="pohon.leadingAvatar({ class: [props.pohon?.leadingAvatar, item.ui?.leadingAvatar] })" />
        </slot>

        <span v-if="get(item, props.labelKey as string) || !!slots.default" :class="pohon.label({ class: [props.pohon?.label, item.ui?.label] })">
    ...
    <template v-if="!!content">
      <TabsContent v-for="(item, index) of items" :key="index" :value="item.value ?? String(index)" :class="pohon.content({ class: [props.pohon?.content, item.ui?.content, item.class] })">
