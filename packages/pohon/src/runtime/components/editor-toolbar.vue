...
import type { PButtonProps, PDropdownMenuProps, DropdownMenuItem, PTooltipProps, PLinkPropsKeys } from '../types'
...
const ui = computed(() => tv({ extend: tv(theme), ...(appConfig.pohon?.editorToolbar || {}) })({
  layout: props.layout
}))
...
function getPButtonProps(item: EditorToolbarItem) {
...
<template>
  <Primitive
    ...
    v-bind="Component !== 'template' ? {
      editor,
      tabindex: -1,
      class: ui.root({ class: pohonProp?.root }),
      ...rootProps,
      options,
      ...$attrs
    } : {
  ...
  >
    <Primitive :as="as" role="toolbar" data-slot="base" :class="ui.base({ class: [pohonProp?.base, props.class] })">
      <template v-for="(group, groupIndex) in groups" :key="`group-${groupIndex}`">
        <div role="group" data-slot="group" :class="ui.group({ class: pohonProp?.group })">
          <template v-for="(item, index) in group" :key="`group-${groupIndex}-${index}`">
            ...
            >
              ...
              >
                <PTooltip v-if="item.tooltip" :disabled="isDisabled(item)" v-bind="{ ...(item.tooltip || {}) }">
                  <PButton :active="isActive(item)" :disabled="isDisabled(item)" v-bind="getPButtonProps(item)" @click="onClick($event, item)" />
                </PTooltip>

                <PButton v-else :active="isActive(item)" :disabled="isDisabled(item)" v-bind="getPButtonProps(item)" @click="onClick($event, item)" />
              </PDropdownMenu>

              <PTooltip v-else-if="item.tooltip" :disabled="isDisabled(item)" v-bind="{ ...(item.tooltip || {}) }">
                <PButton
                  :active="isActive(item)"
                  :disabled="isDisabled(item)"
                  v-bind="getPButtonProps(item)"
                  : pohon="item.ui"
        ...
        <Separator
          ...
          data-slot="separator"
          :class="ui.separator({ class: pohonProp?.separator })"
