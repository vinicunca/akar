...
import type { PricingPlanProps, PricingPlanSlots } from './pricing-plan.vue'
...
const ui = computed(() => tv({ extend: tv(theme), ...(appConfig.pohon?.pricingPlans || {}) }))
...
<template>
  <Primitive :as="as" :data-orientation="orientation" :class="ui({ class: [pohonProp?.base, props.class], compact, scale, orientation })" :style="{ '--count': count }">
    <slot>
      <PPricingPlan
