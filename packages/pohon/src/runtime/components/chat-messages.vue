...
import type { PButtonProps, ChatMessageProps, ChatMessageSlots, PIconProps, PLinkPropsKeys } from '../types'
...
export type ChatMessagesSlots = {
  ...
  default?(props?: {}): VNode[]
  indicator?(props: { pohon: ChatMessages['pohon'] }): VNode[]
  viewport?(props: { pohon: ChatMessages['pohon'], onClick: () => void }): VNode[]
}
...
const ui = computed(() => tv({ extend: tv(theme), ...(appConfig.pohon?.chatMessages || {}) })({
...
<template>
  ...
  >
    ...
    >
      <template #content>
        <slot name="indicator" : pohon="ui">
          <div data-slot="indicator" :class="ui.indicator({ class: pohonProp?.indicator })">
            <span />
            <span />
            <span />
          </div>
    ...
    <Presence :present="showAutoScroll">
      <div :data-state="showAutoScroll ? 'open' : 'closed'" data-slot="viewport" :class="ui.viewport({ class: pohonProp?.viewport })">
        <slot name="viewport" : pohon="ui" :on-click="onAutoScrollClick">
          <PButton
            v-if="autoScroll"
            ...
            :class="ui.autoScroll({ class: pohonProp?.autoScroll })"
