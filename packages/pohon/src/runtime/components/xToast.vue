<script lang="ts">
import type { ToastRootProps, ToastRootEmits } from 'akar'
...
import { ToastRoot, ToastTitle, ToastDescription, ToastAction, ToastClose, useForwardPropsEmits } from 'akar'
...
import UProgress from './xProgress.vue'
...
<template>
  ...
  >
    <slot name="leading">
      <PAvatar v-if="avatar" :size="((props.pohon?.avatarSize || pohon.avatarSize()) as AvatarProps['size'])" v-bind="avatar" :class="pohon.avatar({ class: props.pohon?.avatar })" />
      <PIcon v-else-if="icon" :name="icon" :class="pohon.icon({ class: props.pohon?.icon })" />
    </slot>

    <div :class="pohon.wrapper({ class: props.pohon?.wrapper })">
      <ToastTitle v-if="title || !!slots.title" :class="pohon.title({ class: props.pohon?.title })">
        <slot name="title">
      ...
      <ToastDescription v-if="description || !!slots.description" :class="pohon.description({ class: props.pohon?.description })">
      ...
      <div v-if="orientation === 'vertical' && (actions?.length || !!slots.actions)" :class="pohon.actions({ class: props.pohon?.actions })">
        <slot name="actions">
          <ToastAction v-for="(action, index) in actions" :key="index" :alt-text="action.label || 'Action'" as-child @click.stop>
            <PButton size="xs" :color="color" v-bind="action" />
          </ToastAction>
        </slot>
      </div>
    </div>

    <div v-if="(orientation === 'horizontal' && (actions?.length || !!slots.actions)) || close" :class="pohon.actions({ class: props.pohon?.actions, orientation: 'horizontal' })">
