<script lang="ts">
import type { RadioGroupRootProps, RadioGroupRootEmits } from 'akar'
...
import { RadioGroupRoot, RadioGroupItem as RRadioGroupItem, RadioGroupIndicator, Label, useForwardPropsEmits } from 'akar'
...
<template>
  ...
  >
    <fieldset :class="pohon.fieldset({ class: props.pohon?.fieldset })" v-bind="ariaAttrs">
      <legend v-if="legend || !!slots.legend" :class="pohon.legend({ class: props.pohon?.legend })">
        <slot name="legend">
          {{ legend }}
        </slot>
      </legend>

      <component :is="(!variant || variant === 'list') ? 'div' : Label" v-for="item in normalizedItems" :key="item.value" :class="pohon.item({ class: [props.pohon?.item, item.ui?.item, item.class] })">
        ...
        <div v-if="(item.label || !!slots.label) || (item.description || !!slots.description)" :class="pohon.wrapper({ class: [props.pohon?.wrapper, item.ui?.wrapper] })">
          <component :is="(!variant || variant === 'list') ? Label : 'p'" v-if="item.label || !!slots.label" :for="item.id" :class="pohon.label({ class: [props.pohon?.label, item.ui?.label] })">
            <slot name="label" :item="item" :model-value="(modelValue as RadioGroupValue)">
              {{ item.label }}
            </slot>
          </component>
          <p v-if="item.description || !!slots.description" :class="pohon.description({ class: [props.pohon?.description, item.ui?.description] })">
