<script lang="ts">
import type { TooltipRootProps, TooltipRootEmits, TooltipContentProps, TooltipContentEmits, TooltipArrowProps, TooltipTriggerProps } from 'akar'
...
import { TooltipRoot, TooltipTrigger, TooltipPortal, TooltipContent, TooltipArrow, useForwardPropsEmits } from 'akar'
...
<template>
  <TooltipRoot v-slot="{ open }" v-bind="rootProps" :disabled="!(text || kbds?.length || !!slots.content) || props.disabled">
    ...
    <TooltipPortal v-bind="portalProps">
      <TooltipContent v-bind="contentProps" :class="pohon.content({ class: [!slots.default && props.class, props.pohon?.content] })">
        <slot name="content">
          <span v-if="text" :class="pohon.text({ class: props.pohon?.text })">{{ text }}</span>

          <span v-if="kbds?.length" :class="pohon.kbds({ class: props.pohon?.kbds })">
            <UKbd v-for="(kbd, index) in kbds" :key="index" :size="((props.pohon?.kbdsSize || pohon.kbdsSize()) as KbdProps['size'])" v-bind="typeof kbd === 'string' ? { value: kbd } : kbd" />
          </span>
        </slot>

        <TooltipArrow v-if="!!arrow" v-bind="arrowProps" :class="pohon.arrow({ class: props.pohon?.arrow })" />
      </TooltipContent>
    </TooltipPortal>
  </TooltipRoot>
</template>
