...
export interface EditorMenuOptions<T = any> {
   ...
   */
  items?: MaybeRef<Array<T> | Array<Array<T>> | undefined>;
  /**
...
export function useEditorMenu<T = any>(options: EditorMenuOptions<T>) {
  ...
  function defaultFilter(items: Array<T>, query: string) {
    ...
    for (const item of items) {
      ...
      for (const field of filterFields) {
        const value = getProp(item as any, field);
        if (value == null) {
          continue;
        }
  ...
  const filteredGroups = computed<Array<Array<T>>>(() => {
  ...
  function showMenu() {
    ...
    if (!globalKeyHandler) {
      globalKeyHandler = (event: KeyboardEvent) => {
        if (keyDownHandler) {
          const handled = keyDownHandler({ event });
          if (handled) {
  ...
  if (options.ignoreFilter) {
    stopItemsWatch = watch(() => unref(options.items), (newItems) => {
      ...
      let normalizedItems: Array<T> = [];

      if (newItems?.length) {
        normalizedItems = isArrayOfArray(newItems)
          ? (newItems as Array<Array<T>>).flat()
          : (newItems as Array<T>);
      }
