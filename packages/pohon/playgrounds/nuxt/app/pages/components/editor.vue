...
import type { EditorCustomHandlers, PDropdownMenuItem, PEditorEmojiMenuItem, PEditorSuggestionMenuItem, PEditorToolbarItem } from 'pohon-ui';
...
}]] satisfies Array<Array<EditorToolbarItem<typeof customHandlers>>>);

function imageToolbarItems(editor: any): Array<Array<EditorToolbarItem<typeof customHandlers>>> {
...
function handleItems(editor: any): Array<Array<DropdownMenuItem>> {
...
const suggestionItems = [[{
  ...
  icon: 'i-lucide-separator-horizontal',
}]] satisfies Array<Array<EditorSuggestionMenuItem<typeof customHandlers>>>;
...
const { data: mentionItems } = await useFetch('https://dummyjson.com/users/search?limit=10', {
  ...
  transform: (data: { users: Array<{ id: number; firstName: string; lastName: string; image: string }> }) => {
    return data.users?.map((user) => ({ id: user.id, label: `${user.firstName} ${user.lastName}`, avatar: { src: user.image } })) || [];
  },
  lazy: true,
});

const emojiItems: Array<EditorEmojiMenuItem> = gitHubEmojis.filter((emoji) => !emoji.name.startsWith('regional_indicator_'));
</script>
