...
export function useDashboardExtraMenu(useRootMenus?: ComputedRef<Array<DashboardMenuRecord>>) {
  ...
  const extraRootMenus = ref<Array<DashboardMenuRecord>>([]);
  const route = useRoute();
  const extraMenus = ref<Array<DashboardMenuRecord>>([]);
  ...
  async function handleMixedMenuSelect(menu: DashboardMenuRecord) {
  ...
  async function handleDefaultSelect({ menu, rootMenu }: { menu: DashboardMenuRecord; rootMenu?: DashboardMenuRecord }) {
  ...
  function handleMenuMouseEnter(menu: DashboardMenuRecord) {
    if (!preferences.sidebar.expandOnHover) {
